...
describe("newCommand/deleteCommand", () => {
  it("creates and deletes a service scaffold", async () => {
    const repoRoot = await fs.mkdtemp(path.join(os.tmpdir(), "mono-toolkit-new-"));
    await fs.writeFile(path.join(repoRoot, "package.json"), JSON.stringify({ private: true }));
    ...
    expect(response).toMatchObject({ status: "ok", type: "service", name: "alpha" });
  ...
  it("writes text output for delete", async () => {
    const repoRoot = await fs.mkdtemp(path.join(os.tmpdir(), "mono-toolkit-delete-"));
    await fs.writeFile(path.join(repoRoot, "package.json"), JSON.stringify({ private: true }));
  ...
  it("creates a feature scaffold", async () => {
    const repoRoot = await fs.mkdtemp(path.join(os.tmpdir(), "mono-toolkit-feature-"));
    await fs.writeFile(path.join(repoRoot, "package.json"), JSON.stringify({ private: true }));
    const serviceDir = path.join(repoRoot, "back", "services", "alpha", "src", "main", "java", "features");
    await fs.mkdir(serviceDir, { recursive: true });

    const { context, stdout } = createContext(repoRoot);
    const runNew = await loadCommandFn(newCommand);
    await runNew.call(context, { json: true, in: "back/services/alpha" }, "feature", "orders");
