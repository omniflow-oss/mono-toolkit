...
describe("sanitizeName", () => {
  ...
  it("rejects overly long names", () => {
    expect(() => sanitizeName("name-that-is-way-too-long", policies)).toThrow(ToolkitError);
    expect(() => sanitizeName("name-that-is-way-too-long", policies)).toThrowError(
      expect.objectContaining({ code: ExitCode.InvalidConfig })
    );
  });
});
