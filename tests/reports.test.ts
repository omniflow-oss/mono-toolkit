...
describe("reports", () => {
	...
	it("writes summary report", async () => {
		const repoRoot = await fs.mkdtemp(
			path.join(os.tmpdir(), "mono-toolkit-summary-"),
		);
		...
		await writeSummaryReport(repoRoot, {
			...
			status: "success",
			scopes: [
				{ id: "back:service:alpha", tasks: [{ id: "lint", exitCode: 0 }] },
			],
		});
