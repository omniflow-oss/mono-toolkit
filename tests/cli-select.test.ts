...
const scopes: ScopeRecord[] = [
  { id: "back:service:alpha", type: "service", path: "back/services/alpha", profile: "default", tags: ["api"] },
...
describe("selectScopes", () => {
  it("selects by scope id", async () => {
    const result = await selectScopes({
      ...
      config: baseConfig,
      scopes,
      selection: { scope: "back:service:beta", tag: undefined, changed: true, all: false }
    });
