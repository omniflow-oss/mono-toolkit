...
describe("discoverScopes", () => {
  it("discovers scopes and applies overrides", async () => {
    const repoRoot = await fs.mkdtemp(path.join(os.tmpdir(), "mono-toolkit-scopes-"));
    ...
    expect(scopes).toEqual(
      expect.arrayContaining([
        expect.objectContaining({ id: "back:service:alpha", tags: ["api"], port: 8080 }),
        expect.objectContaining({ id: "back:lib:core", profile: "maven" })
      ])
    );
    expect(scopes.find((scope) => scope.id === "back:service:beta")).toBeUndefined();
    ...
    const alphaDefaultPort = scopes.find((scope) => scope.id === "back:service:alpha")?.port;
    expect(alphaDefaultPort).toBe(8080);
  });

  it("assigns deterministic ports when missing", async () => {
    const repoRoot = await fs.mkdtemp(path.join(os.tmpdir(), "mono-toolkit-ports-"));
    await fs.mkdir(path.join(repoRoot, "back/services/alpha"), { recursive: true });
