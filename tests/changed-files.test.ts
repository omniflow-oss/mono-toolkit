...
import { getChangedFiles, mapChangedFilesToScopes } from "../src/changed/changed";
...
describe("getChangedFiles", () => {
  it("normalizes changed file paths", async () => {
    ...
    expect(files).toEqual(["back/services/alpha/src/index.ts", "config/tools/file"]);
  });

  it("throws when git diff fails", async () => {
    vi.mocked(resolveBaseRef).mockResolvedValue("base");
    vi.mocked(execCommand).mockResolvedValue({ exitCode: 1, stdout: "", stderr: "bad" });

    await expect(
      getChangedFiles({ defaultBranch: "main", allowFetchBase: false, cwd: "/repo" })
...
describe("mapChangedFilesToScopes", () => {
  it("handles special prefixes and scope matches", () => {
    ...
    const result = mapChangedFilesToScopes(
      ["contracts/spec.yaml", "docs/readme.md", "back/services/alpha/src/index.ts"],
      scopes,
      config
    );
