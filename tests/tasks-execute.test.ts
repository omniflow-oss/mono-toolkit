...
describe("executePipeline", () => {
	it("returns dry-run commands", async () => {
		...
		expect(results[0].tasks[0].stdout).toBe(
			"pnpm -C back/services/alpha echo hi",
		);
		expect(results[0].tasks[0].command).toEqual([
			"pnpm",
			"-C",
			"back/services/alpha",
			"echo",
	...
	it("skips cached tasks", async () => {
		const repoRoot = await fs.mkdtemp(
			path.join(os.tmpdir(), "mono-toolkit-cache-"),
		);
	...
	it("runs task dependencies in order", async () => {
		...
		expect(results[0].tasks.map((task) => task.taskId)).toEqual([
			"lint",
			"build",
		]);
	});
