...
const typeRoots = (paths: { backServices: string; backLibs: string; frontApps: string; frontPackages: string }) => ({
...
>({
  parameters: {
    ...
    flags: {
      in: { kind: "parsed", brief: "Target service scope for feature", parse: String, optional: true },
  ...
  func: async function (flags, type: string, name: string) {
    ...
    try {
      ...
      if (type === "feature") {
        ...
        const scope = scopes.find((entry) => entry.id === flags.in || entry.path === flags.in);
        if (!scope) {
          throw new Error(`Scope not found: ${flags.in}`);
        }
        targetDir = path.join(repoRoot, scope.path, "src", "main", "java", "features", sanitized);
      } else {
      ...
      if (flags.json) {
        writeJson(context, { status: "ok", type, name: sanitized, path: targetDir });
      } else {
