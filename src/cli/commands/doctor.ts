...
export const doctorCommand = buildCommand({
  ...
  func: async function (flags) {
    ...
    try {
      ...
      const gitCheck = await execCommand("git", ["--version"], { cwd: repoRoot });
      checks.push({ name: "git", ok: gitCheck.exitCode === 0, details: gitCheck.stderr || gitCheck.stdout });

      const dockerCheck = await execCommand("docker", ["--version"], { cwd: repoRoot });
      checks.push({ name: "docker", ok: dockerCheck.exitCode === 0, details: dockerCheck.stderr || dockerCheck.stdout });

      const composeCheck = await execCommand("docker", ["compose", "version"], { cwd: repoRoot });
      checks.push({ name: "docker-compose", ok: composeCheck.exitCode === 0, details: composeCheck.stderr || composeCheck.stdout });
      ...
      try {
        ...
        if (!flags.fix) {
          throw new ToolkitError("Cache directories not writable", ExitCode.TaskFailed);
        }
      }
