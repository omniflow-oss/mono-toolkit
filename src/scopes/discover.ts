...
const toDeterministicPort = (scopeId: string, base = 4000, range = 10000): number => {
  let hash = 0;
  ...
  for (const rootEntry of scopeRoots(paths)) {
    ...
    for (const name of scopes) {
      ...
      result.push({
        ...
        tags: override.tags ?? [],
        port: override.port ?? (rootEntry.type === "service" ? toDeterministicPort(scopeId) : undefined),
        deps: override.deps
      });
    }
  }

  const addSpecialScope = async (id: string, type: ScopeRecord["type"], relativePath: string, profile = "default") => {
