...
export const getChangedFiles = async (options: {
  ...
  const result = await execCommand("git", ["diff", "--name-only", `${baseRef}...HEAD`], {
    cwd: options.cwd
  });
  if (result.exitCode !== 0) {
    throw new ToolkitError("Unable to compute changed files", ExitCode.GitMissing, {
...
export const mapChangedFilesToScopes = (
  ...
  for (const file of changedFiles) {
    ...
    const match = scopes.find((scope) => file.startsWith(normalizePath(scope.path) + "/"));
    if (match) {
      result.add(match.id);
    }
  }
...
export const getChangedScopes = async (options: {
  ...
  return mapChangedFilesToScopes(changedFiles, options.scopes, options.changedConfig);
};
