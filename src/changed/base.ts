...
export const resolveBaseRef = async (options: {
  ...
  const envRef = envBaseRefs.map((name) => process.env[name]).find((value) => Boolean(value));
  if (envRef) {
    return envRef;
  }

  const base = options.base ?? options.defaultBranch;
  const mergeBase = await execCommand("git", ["merge-base", "HEAD", base], { cwd: options.cwd });
  ...
  const fetchResult = await execCommand("git", ["fetch", "origin", base, "--depth=1"], {
    cwd: options.cwd
  });
  ...
  const mergeBaseAfterFetch = await execCommand("git", ["merge-base", "HEAD", base], { cwd: options.cwd });
  if (mergeBaseAfterFetch.exitCode !== 0) {
    throw new ToolkitError("Unable to compute merge-base", ExitCode.GitMissing, {
